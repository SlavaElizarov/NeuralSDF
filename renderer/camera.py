...
class Camera():
    ...
    def __init__(self,
        ...
        R,T = look_at_view_transform(self.dist, self.elev, self.azim, device = self.device)
        self.rotation = R
        self.origin = T
    ...
    def emit_rays(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ray_directions /= torch.linalg.norm(ray_directions, dim=-1, keepdim=True) # normalize vectors
