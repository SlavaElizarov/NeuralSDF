...
class Siren(nn.Sequential, SDF):
    ...
    def geometric_init(self):
        ...
        def first_layer_mfgi_init(m):
            ...
            with torch.no_grad():
                if hasattr(m, "weight"):
                    ...
                    W0_new = torch.cat(weights, axis=0)  # type: ignore
        ...
        self[0].linear.apply(first_layer_mfgi_init)  # type: ignore
        self[1].linear.apply(second_layer_mfgi_init)  # type: ignore
        self[-3].linear.apply(second_last_layer_geom_sine_init)  # type: ignore
